...
to Mavlink and sends it out again on Bluetooth.
 ...
 2020-7  1.10 for esp32 with bluetooth by ragelancer
 */
...
//String MACadd = "00:0C:BF:05:3B:74";
uint8_t address[6]  = {0x00, 0x0C, 0xBF, 0x05, 0x3B, 0x74};
...
//************* Pin Assignments
// Serial1 Frsky telemetry in     RX = 13   (TX = 14 not used)
// Serial to monitor print        USB
// SerialBT for Mav BT out         Bluetooth
...
void setup()
{  
  Serial1.begin(57600, SERIAL_8N1, 13, 14, true);       // Frsky Telemetry input      - TX2 & RX2, Pins A2  & A3
 // SerialBT.begin(57600, SERIAL_8N1, 16, 17, false);       // Mavlink Telemetry output   - TX3 & RX3, Pins B10 & B11
  Serial.begin(115200);       // Flash & Print Output       - TX1 & RX1, Pins A9  & A10
  SerialBT.setPin("1234");
  SerialBT.begin("Mavlink", true); 
  //SerialBT.setPin(pin);
...
void EncodeMavlink() {     
  ...
  if (GPS_5002_flag && velyaw_5005_flag) {  
    ...
    mavlink_msg_gps_raw_int_pack(system_id, component_id,  &msg,
        utime_usec, fix_type, lat, lon, altmsl, eph, epv, vel, cog, satellites_visible);
    len = mavlink_msg_to_send_buffer(buf, &msg);
    SerialBT.write(buf, len); 
